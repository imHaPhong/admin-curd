...
import { ProjectStatusRow } from "./tech-stack-row";
...
export function TechStackTable() {
  ...
  const [listProjectStatus, setListProjectStatus] = useState<TechStack[]>([]);

  useEffect(() => {
    async function getProjectStatuss() {
      const ProjectStatuss = await apiClientBrowser.get("http://localhost:8080/tech-stack");
      setListProjectStatus(ProjectStatuss.data as TechStack[]);
    }
    getProjectStatuss();
  }, []);
  return (
